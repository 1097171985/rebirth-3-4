...
static const CGFloat kPreloadViewCount = MAXFLOAT;
...
@property(nonatomic, strong)NSString *orign;
...
-(instancetype)initWithFrame:(CGRect)frame withOrign:(NSString *)orign{
...
- (void)loadNextView
{
    if (self.dataSource && [self.dataSource respondsToSelector:@selector(cardContainerViewNumberOfViewInIndex:)]) {
        ...
        if ([self.orign isEqualToString:@"DingzhiRoute"]) {
            
            maxNum = _currentIndex;
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
     ...
     NSLog(@"wwww== %lu",(unsigned long)_currentViews.count);
    if (!_isInitialAnimation) { return; }
    ...
    if (gesture.state == UIGestureRecognizerStateChanged) {
        ...
        CGPoint movedPoint = CGPointMake(gesture.view.center.x , gesture.view.center.y + point.y);
...
//         CGAffineTransformMakeRotation((gesture.view.center.x - _cardCenterX) / _cardCenterX * (_moveSlope * (M_PI / 20)))];
    ...
    if (gesture.state == UIGestureRecognizerStateEnded ||
        ...
        if (direction == YSLDraggableDirectionDefault) {
            ...
            NSLog(@" 111 ===%ld",(long)_currentIndex);
...
- (void)cardViewTap:(UITapGestureRecognizer *)gesture
{
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(cardContainerView:didSelectAtIndex:draggableView:)]) {
        [self.delegate cardContainerView:self didSelectAtIndex:_currentIndex%3 draggableView:gesture.view];
    }
}
