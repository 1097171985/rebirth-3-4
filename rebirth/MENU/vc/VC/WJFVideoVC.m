...
#define kTrans kDuration/TotalTime
//获取屏幕 宽度、高度
#define WIDTH ([UIScreen mainScreen].bounds.size.width)
#define HEIGHT ([UIScreen mainScreen].bounds.size.height)

static float kDuration = 0;
...
@interface WJFVideoVC ()<AVCaptureFileOutputRecordingDelegate>
{
...
@property(nonatomic, strong)PhotographBtu *photographBtu;
...
@property(nonatomic, strong)DACircularProgressView  *dacirView;


@property(nonatomic, strong)NSString *levelStr;

@property(nonatomic, strong)UIButton *vip1Btu;
@property(nonatomic, strong)UIButton *vip2Btu;
@property(nonatomic, strong)UIButton *vip3Btu;

@end

@implementation WJFVideoVC
...
-(void)loadNetWork{
    ...
    [WJFCollection getWithURLString:@"http://www.rempeach.com/rebirth/api/AppApi/receive" parameters:signDict success:^(id responseObject) {
        ...
        self.levelStr = responseObject[@"data"][@"user_level"];
        if ([self.levelStr isEqualToString:@"0"]) {
         
            self.vip3Btu.layer.borderColor = [NSString colorWithHexString:@"666666"].CGColor;
            
            [self.vip3Btu setTitleColor:[NSString colorWithHexString:@"666666"] forState:UIControlStateNormal];
            
            self.vip3Btu.userInteractionEnabled = NO;
...
-(void)loadThreeBtu{
    ...
    self.vip1Btu = [UIButton buttonWithType:UIButtonTypeCustom];
    self.vip1Btu.frame = CGRectMake(40,self.videoView.frame.origin.y+self.videoView.frame.size.height+120, 136/2, 40);
    [self.vip1Btu setTitle:@"15秒" forState:UIControlStateNormal];
    self.vip1Btu.layer.masksToBounds = YES;
    self.vip1Btu.layer.cornerRadius = 4;
    self.vip1Btu.layer.borderWidth = 1;
    self.vip1Btu.layer.borderColor = [UIColor whiteColor].CGColor;
    [self.vip1Btu addTarget:self action:@selector(selectVipTime:) forControlEvents:UIControlEventTouchUpInside];
    
    [self.view addSubview:self.vip1Btu];
    
    
    
    self.vip2Btu = [UIButton buttonWithType:UIButtonTypeCustom];
    self.vip2Btu.frame = CGRectMake(self.vip1Btu.frame.origin.x+self.vip1Btu.frame.size.width+45,self.videoView.frame.origin.y+self.videoView.frame.size.height+120, 136/2, 40);
    [self.vip2Btu setTitle:@"30秒" forState:UIControlStateNormal];
    self.vip2Btu.layer.masksToBounds = YES;
    self.vip2Btu.layer.cornerRadius = 4;
    self.vip2Btu.layer.borderWidth = 1;
    self.vip2Btu.layer.borderColor = [UIColor whiteColor].CGColor;
    [self.vip2Btu addTarget:self action:@selector(selectVipTime:) forControlEvents:UIControlEventTouchUpInside];
    
    [self.view addSubview:self.vip2Btu];
    
    
    self.vip3Btu = [UIButton buttonWithType:UIButtonTypeCustom];
    self.vip3Btu.frame = CGRectMake(self.vip2Btu.frame.origin.x+self.vip2Btu.frame.size.width+45,self.videoView.frame.origin.y+self.videoView.frame.size.height+120, 136/2, 40);
    [self.vip3Btu setTitle:@"60秒" forState:UIControlStateNormal];
    self.vip3Btu.layer.masksToBounds = YES;
    self.vip3Btu.layer.cornerRadius = 4;
    self.vip3Btu.layer.borderWidth = 1;
    self.vip3Btu.layer.borderColor = [UIColor whiteColor].CGColor;
    [self.vip3Btu addTarget:self action:@selector(selectVipTime:) forControlEvents:UIControlEventTouchUpInside];
    
    [self.view addSubview:self.vip3Btu];
    
}


-(void)selectVipTime:(UIButton *)btu{
    
    NSLog(@"%f===%f",WIDTH*2,self.videoView.frame.size.height);
    self.vip1Btu.hidden = YES;
    
    self.vip2Btu.hidden = YES;
    
    self.vip3Btu.hidden = YES;
    
    self.photographBtu.hidden = NO;
    if ([btu.titleLabel.text isEqualToString:@"15秒"]) {
        
        TotalTime = 15.0;
    }else if ([btu.titleLabel.text isEqualToString:@"30秒"]){
        TotalTime = 30.0;
        
    }else if ([btu.titleLabel.text isEqualToString:@"60秒"]){
        TotalTime = 60.0;
...
-(void)progess:(NSTimer *)timer{
    ...
    number ++;
    NSLog(@"%f",number/60.0);
    self.photographBtu.photoTimeProgressView.persentage = number/60.0;
    
}
-(void)loadPhotograph{
    
    self.photographBtu = [[PhotographBtu alloc]initWithFrame:CGRectMake(WIDTH/2-75, self.videoView.frame.origin.y+self.videoView.frame.size.height+120, 150, 150)];
    self.photographBtu.hidden  = YES;
    self.photographBtu.photoTimeProgressView.persentage = 0;
    [self.view addSubview:self.photographBtu];
...
-(void)loadVideoView{
    ...
    self.videoView = [[UIView alloc]initWithFrame:CGRectMake(0, 64, WIDTH,562/2)];
...
- (void)initUI
{
    ...
    self.dacirView = [[DACircularProgressView alloc]initWithFrame:CGRectMake(0, 0, self.tapBtn.frame.size.width, self.tapBtn.frame.size.height)];
    
    [self.tapBtn addSubview:self.dacirView];
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event
{
    ...
    NSLog(@"=======%d",condition);
    if (condition) {
        if (self.photographBtu.photoTimeProgressView.persentage > 0.6) {
            //录制完成
            [self recordComplete];
        }
    }else{
...
- (BOOL)isInBtnRect:(CGPoint)point
{
    ...
    return  (x>self.photographBtu.left && x<self.photographBtu.right) && (y>self.photographBtu.top && y<self.photographBtu.bottom);
}
...
- (void)refresh:(CADisplayLink *)link
{
    ...
    self.dacirView.progress = kTrans;
    NSLog(@"%f",kDuration);
    self.photographBtu.photoTimeProgressView.persentage = kTrans;
    kDuration = kDuration + 1/60.0;
...
- (void)pushToPlay:(NSURL *)url
{
    ...
    PostVideoPlayerController *postVC = [[PostVideoPlayerController alloc]init];
    postVC.videoUrl = url;
