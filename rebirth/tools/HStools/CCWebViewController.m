...
-(void)creatNavigation {
    ...
    UIView *naviView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, kScreenWidth, NAV_BAR_HEIGHT)];
   // naviView.backgroundColor = [NSString colorWithHexStri];
    [self.view addSubview:naviView];
    UIButton *returnBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    [returnBtn setImage:[UIImage imageNamed:@"back_arrow@2x"] forState:UIControlStateNormal];
    returnBtn.frame = CGRectMake(0,kStatusBarHeight,50,NAV_BAR_HEIGHT-kStatusBarHeight);
    [naviView addSubview:returnBtn];
    [returnBtn addTarget:self action:@selector(btn_clickBack) forControlEvents:UIControlEventTouchUpInside];
    UILabel *titlelb = [[UILabel alloc] initWithFrame:CGRectMake(50, kStatusBarHeight, kScreenWidth-100, NAV_BAR_HEIGHT-kStatusBarHeight)];
...
- (void)configUI {
    ...
    // 进度条
    UIProgressView *progressView = [[UIProgressView alloc] initWithFrame:CGRectMake(0, NAV_BAR_HEIGHT, self.view.frame.size.width, 0)];
    progressView.tintColor = [NSString colorWithHexString:@"e4c675"];
    ...
    // 网页
    if (IOS8x) {
        WKWebView *wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, NAV_BAR_HEIGHT, kScreenWidth, kScreenHeight-NAV_BAR_HEIGHT)];
