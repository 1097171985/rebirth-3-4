...
- (void)commonInit {
    ...
    self.selectionIndicatorEdgeInsets = UIEdgeInsetsMake(0, 0, 0, 0);
...
- (void)setSectionTitles:(NSArray *)sectionTitles {
    ...
    _sectionSelectedTitles = sectionTitles;
    [self setNeedsLayout];
}
...
- (CGSize)measureTitleAtIndex:(NSUInteger)index {
    id title = self.selected ? self.sectionSelectedTitles[index] : self.sectionTitles[index];
    ...
    // 用于暂时适配iOS10的问题，之后这个组件需重写。
    if ( size.width < 35) {
        size = CGSizeMake(size.width+5, size.height);
    }
...
- (NSAttributedString *)attributedTitleAtIndex:(NSUInteger)index {
    BOOL selected = (index == self.selectedSegmentIndex) ? YES : NO;
    id title = selected ? self.sectionSelectedTitles[index] : self.sectionTitles[index];
    if (!title) {
        title = @"";
    }
...
- (CGRect)frameForSelectionIndicator {
    ...
    if (self.selectionIndicatorWidth > 0) {
        sectionWidth = self.selectionIndicatorWidth;
    }
    else {
        if (self.type == HMSegmentedControlTypeText) {
...
- (NSDictionary *)resultingTitleTextAttributes {
    NSMutableDictionary *resultingAttrs = [NSMutableDictionary dictionaryWithDictionary:@{NSForegroundColorAttributeName: [UIColor blueColor], NSFontAttributeName:[UIFont systemFontOfSize:15]}];
